...
const { maskAccountNumber } = require('./validators');

/**
 * PDF Generation Utility for EZFinanz Loan Sanction & Disbursement Letter
 */
const generateSanctionLetterPDF = (application, res) => {
  const { applicationNumber, kyc, selectedTerms, bankDetails, disbursement } = application;
  const applicantName = kyc?.fullName || application.user?.fullName || 'Valued Customer';
  ...
  const issueDate = disbursement?.disbursedAt
    ? new Date(disbursement.disbursedAt).toLocaleDateString('en-IN')
