...
import { CheckCircle2, Clock, AlertTriangle, ArrowRight, ShieldCheck, FileCheck, Landmark, DollarSign, Camera } from 'lucide-react';
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-slate-600">Fetching application timeline...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <main className="flex-1 max-w-xl w-full mx-auto px-4 py-16 flex items-center justify-center">
          <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center space-y-4 w-full">
            ...
            <h2 className="text-xl font-bold text-slate-900">No Active Application Found</h2>
            ...
            <div className="pt-2">
              <Link
                to="/verify-contact"
                className="inline-flex items-center space-x-2 py-3 px-6 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-sm rounded-xl shadow-md transition-all"
              >
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
          <div className="flex items-start justify-between">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Application Number</span>
              <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight mt-0.5">{applicationNumber}</h2>
              <p className="text-xs text-slate-500 mt-1">Submitted on {new Date(application.createdAt).toLocaleDateString()}</p>
            </div>

            <div className="text-right">
              {currentStage === 'UNDER_ADMIN_REVIEW' ? (
                <span className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full text-xs font-extrabold bg-amber-100 text-amber-800 border border-amber-200">
                  <Clock className="w-4 h-4 animate-spin text-amber-600" />
                  <span>UNDER ADMIN REVIEW</span>
                </span>
              ) : currentStage === 'SELFIE_APPROVED' ? (
                <span className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full text-xs font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-200">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                  <span>APPROVED - AWAITING DISBURSEMENT</span>
                </span>
              ) : currentStage === 'SELFIE_REJECTED' ? (
                <span className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full text-xs font-extrabold bg-red-100 text-red-800 border border-red-200">
                  <AlertTriangle className="w-4 h-4 text-red-600" />
                  <span>ACTION REQUIRED</span>
                </span>
              ) : (
                <span className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full text-xs font-extrabold bg-blue-100 text-blue-800 border border-blue-200">
        ...
        {/* Action Prompt if not completed */}
        {currentStage === 'SELFIE_REJECTED' && (
          <div className="p-5 bg-red-50 border border-red-200 rounded-2xl flex items-center justify-between">
            <div>
              <h4 className="font-extrabold text-red-900 text-base">Selfie Photo Rejected</h4>
              <p className="text-xs text-red-700 mt-0.5">Reason: {selfie?.rejectionReason}</p>
            </div>
            <Link
              to="/selfie-upload"
              className="py-2.5 px-5 bg-red-600 hover:bg-red-700 text-white font-bold text-xs rounded-xl shadow-sm transition-all"
            >
              Re-upload Selfie
            </Link>
          </div>
        ...
        {currentStage !== 'UNDER_ADMIN_REVIEW' && currentStage !== 'SELFIE_APPROVED' && currentStage !== 'DISBURSED' && currentStage !== 'SELFIE_REJECTED' && (
          <div className="p-5 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center justify-between">
            <div>
              <h4 className="font-extrabold text-emerald-950 text-base">Continue Loan Application</h4>
              <p className="text-xs text-emerald-800 mt-0.5">Your progress has been saved. Pick up right where you left off.</p>
            </div>
            <Link
              to={getNextStepRoute(currentStage)}
              className="py-2.5 px-6 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-xl shadow-md transition-all flex items-center space-x-2"
            >
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
          <h3 className="font-bold text-slate-900 text-base border-b border-slate-100 pb-3">Application Milestones</h3>
            ...
            {[
              ...
              <div key={i} className="flex items-start space-x-3">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                  milestone.done ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400 border border-slate-200'
                }`}>
                  <CheckCircle2 className="w-4 h-4" />
                </div>
                <div className="flex-1 border-b border-slate-100 pb-3">
                  <h4 className={`font-semibold ${milestone.done ? 'text-slate-900 font-bold' : 'text-slate-500'}`}>{milestone.title}</h4>
                  <p className="text-xs text-slate-500 mt-0.5">{milestone.desc}</p>
                </div>
              </div>
            ))}
          </div>
